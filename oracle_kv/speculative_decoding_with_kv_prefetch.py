...
def prefetch_kv(draft_model, target_model, draft_indices_history, fetch_ratio: float = PREFETCH_RATIO):
    """
    ...
    # 3. Frequency Count over logical pages
    ...
    num_to_fetch = max(1, int(len(unique_logical) * fetch_ratio))
...
def main():
    ...
    print(f"Prefetch ratio: {PREFETCH_RATIO}")
    print(f"Prefetch window stride: {PREFETCH_WINDOW_STRIDE}")
    ...
    # Window counter for controlling prefetch frequency
    window_idx = 0

    # Side stream for async prefetch.
    # Use default priority (0); some CUDA setups don't accept positive priorities.
